...
Format	Target Hardware	Benefit
GGUF (Q2-Q8)	AMD and VRAM poor	Best for limited deployment.
...
Because the 55GB RAMDisk consumes a large portion of system memory, follow these rules:
    ...
    Clear First: Use the "Clear RAMDisk" button before starting a new FP8 conversion if a large model is already present.

    Move to SSD: Once a conversion is finished, click "Move RD Models to SSD" to free up system RAM for inference.

    Vitals Monitor: Keep an eye on the Vitals bar at the top of the GUI to ensure RAM usage stays below 90%.
